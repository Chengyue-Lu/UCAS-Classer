...
- `v1.0.4` 新增的自动侧收 MVP 不是 `runtime-shared` 单独可完成的功能。
- 共享层只负责侧收设置 UI 与前端状态表现；真正的窗口侧收命令与 moved/resized 判定属于 `package-shell`。
- 因此本轮已同步更新开发端和 package 端壳层 `main.rs`，保证双端窗口行为一致。
- 打包端系统路径存储约束保持不变，侧收能力没有改变 `UCAS_DATA_ROOT / UCAS_CONFIG_ROOT` 的来源与落点。
